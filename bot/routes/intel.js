 ...
 * independent analysis, track record, webhooks, news, ws, pnl, trades
 */
module.exports = function createIntelRoutes(deps) {
  ...
  router.get('/odds/status', (req, res) => res.json(oddsApi.getStatus()));
  ...
  router.post('/odds/fetch', async (req, res) => {
    try {
      ...
      res.json({
        status: 'ok', bookmakerEvents: events.length, polymarketMatches: matchArr.length,
  ...
  router.post('/accas/build', async (req, res) => {
    ...
    try {
      ...
      if (picksTracker && result.accas?.length > 0) picksTracker.snapshotAccas(result.accas);
      res.json(result);
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/accas/top', (req, res) => {
    ...
    res.json(accaBuilder.getTopAccas(parseInt(req.query.limit) || 10));
  });
  ...
  router.get('/public/accas', (req, res) => {
    ...
    const publicAccas = (data.accas || []).slice(0, 5).map(a => ({
      grade: a.grade, numLegs: a.numLegs, combinedOdds: a.combinedOdds,
      evPercent: a.evPercent, crossSport: a.crossSport,
      legs: a.legs.map(l => ({ match: l.match, pick: l.pick, odds: l.odds, sport: l.sport, betType: l.betType })),
      hypothetical: a.hypothetical, generatedAt: a.generatedAt,
    }));
  ...
  router.get('/accas/clv', (req, res) => {
    if (!accaBuilder) return res.status(503).json({ error: 'Acca builder not available' });
    try { res.json(accaBuilder.analyzeCLV()); }
    catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/accas/movements', (req, res) => {
    ...
    try {
      const data = accaBuilder.getAccas();
      let lineHistory = {};
      try { lineHistory = JSON.parse(fs.readFileSync(path.join(LOGS_DIR, 'line-history.json'), 'utf-8')); } catch { /* ok */ }
      const recentMoves = Object.entries(lineHistory)
        .map(([key, snapshots]) => ({ key, snapshots: snapshots.slice(-5), count: snapshots.length }))
        .filter(m => m.count > 1).sort((a, b) => b.count - a.count).slice(0, 20);
      res.json({ movements: data.stats?.lineMovements || 0, steamMoves: data.stats?.steamMoves || 0, recentMoves });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/picks/history', (req, res) => {
    ...
    res.json(picksTracker.getHistory(parseInt(req.query.page) || 1, parseInt(req.query.limit) || 50));
  });
  ...
  router.post('/picks/resolve', async (req, res) => {
    ...
    try {
      ...
      res.json(await picksTracker.checkAndResolve(apiKey));
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/picks/learning', (req, res) => {
    ...
    res.json(picksLearner.getLearningReport());
  });
  ...
  router.get('/public/picks/learning', (req, res) => {
    ...
    res.json({
      totalResolved: report.totalResolved || 0, overallWinRate: report.overallWinRate || 0,
      overallROI: report.overallROI || 0, insights: report.insights || [], learnedAt: report.learnedAt,
    });
  });
  ...
  router.get('/learning/insights', (req, res) => {
    try {
      res.json(paperTrader.getLearningInsights ? paperTrader.getLearningInsights() : { status: 'Not available' });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/learning/thresholds', (req, res) => {
    try {
      ...
      for (const s of strategies) {
        thresholds[s] = (paperTrader.getLearnedThreshold ? paperTrader.getLearnedThreshold(s) : null) || { status: 'Insufficient data (need 10+ resolved trades)' };
      }
      res.json(thresholds);
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.post('/learning/cycle', (req, res) => {
    try {
      if (!paperTrader.runLearningCycle) return res.status(501).json({ error: 'Learning cycle not available' });
      paperTrader.runLearningCycle();
      res.json({ status: 'ok', message: 'Learning cycle executed', insights: paperTrader.getLearningInsights() });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });

  router.get('/learning/signals', (req, res) => {
    try {
      const insights = paperTrader.getLearningInsights ? paperTrader.getLearningInsights() : {};
      res.json({ signalAccuracy: insights.signalAccuracy || {}, topSignals: insights.topSignals || [], weakSignals: insights.weakSignals || [] });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });

  // ─── Intelligence: Market Movers ───────────────────────────────────
  router.get('/intel/movers', (req, res) => {
    res.json(alerts.getMarketMovers(markets.getCachedMarkets(), parseInt(req.query.limit) || 10));
  });

  router.get('/intel/credibility', (req, res) => res.json(signalTracker.getReport()));

  // ─── Intelligence: Cross-Platform Divergences ──────────────────────
  router.get('/intel/divergences', (req, res) => {
    try {
      const matches = oddsApi.matchAllMarkets(markets.getCachedMarkets());
      ...
      res.json({ count: divergences.length, divergences, timestamp: new Date().toISOString() });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/intel/compare/:conditionId', (req, res) => {
    try {
      ...
      res.json({ conditionId: condId, polymarket: polyMarket || null, kalshi: kalshiMkt || null, bookmaker: bookmaker || null, signals, credibilityBadge: badge, watched: watchlist.isWatched(condId) });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/intel/independent/edges', (req, res) => {
    ...
    if (minQuality > 0) edges = edges.filter(e => (e.edgeQuality || 0) >= minQuality);
    res.json({
  ...
  router.get('/intel/independent/cached', (req, res) => {
    ...
    if (minQuality > 0) results = results.filter(e => (e.edgeQuality || 0) >= minQuality);
    res.json({
      count: results.length, results,
      lastScan: scanState.lastScanTime ? scanState.lastScanTime.toISOString() : null,
      nextScan: scanState.lastScanTime ? new Date(scanState.lastScanTime.getTime() + scanState.SCAN_INTERVAL).toISOString() : null,
      scanRunning: scanState.scanRunning, totalCached: scanState.cachedScanResults.length,
    });
  });
  ...
  router.get('/intel/independent/scanner', (req, res) => {
    res.json({
      ...
      lastScan: scanState.lastScanTime?.toISOString() || null,
      nextScan: scanState.lastScanTime ? new Date(scanState.lastScanTime.getTime() + scanState.SCAN_INTERVAL).toISOString() : null,
  ...
  router.get('/intel/independent/:conditionId', async (req, res) => {
    if (!independentSignals) return res.status(503).json({ error: 'Independent signals module not loaded' });
    try {
      const market = markets.getCachedMarkets().find(m => m.conditionId === req.params.conditionId);
      if (!market) return res.status(404).json({ error: 'Market not found' });
      res.json(await independentSignals.getIndependentSignal(market));
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.post('/intel/independent/llm/analyze', async (req, res) => {
    ...
    try {
      ...
      if (!market) return res.status(404).json({ error: 'Market not found' });
      res.json(await independentSignals.llm.analyzeMarket(market, headlines || []));
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.post('/intel/independent/batch', async (req, res) => {
    ...
    try {
      const body = req.body || {};
      const results = await independentSignals.batchAnalyze(markets.getCachedMarkets(), {
        maxMarkets: body.maxMarkets || body.count || 10, skipLLM: body.skipLLM || false,
      });
      const entries = [];
      ...
      if (strongEdges.length > 0 && webhooks.fire) webhooks.fire(strongEdges).catch(() => {});
      res.json({ count: entries.length, results: entries.sort((a, b) => (b.absDivergence || 0) - (a.absDivergence || 0)), timestamp: new Date().toISOString() });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/intel/calibration', (req, res) => {
    ...
    const avgQuality = edges.length > 0 ? Math.round(edges.reduce((sum, e) => sum + (e.edgeQuality || 0), 0) / edges.length) : 0;
    const sourceHits = {};
    for (const e of edges) { const c = e.sourceCount || e.validatedSourceCount || 0; sourceHits[c] = (sourceHits[c] || 0) + 1; }
    res.json({
      calibration, metrics: {
        totalEdges: edges.length, last24h: last24h.length, last7d: last7d.length, avgQuality,
        qualityDistribution: qualityDist, sourceHitRates: sourceHits,
  ...
  router.get('/public/track-record', (req, res) => {
    try {
      ...
      let allEdges = [];
      try { allEdges = JSON.parse(fs.readFileSync(resFile, 'utf8')).resolutions || []; } catch { /* empty */ }
      const pending = allEdges.filter(e => !e.resolved);
      const resolved = allEdges.filter(e => e.resolved);
      const gradeDistribution = {}, sourceCountDist = {};
      for (const e of allEdges) { const g = e.edgeGrade || '?'; gradeDistribution[g] = (gradeDistribution[g] || 0) + 1; }
      for (const e of allEdges) { const b = (e.sourceCount || 0) >= 6 ? '6+' : String(e.sourceCount || 0); sourceCountDist[b] = (sourceCountDist[b] || 0) + 1; }
      const activeEdges = pending.sort((a, b) => new Date(b.recordedAt) - new Date(a.recordedAt)).map(e => ({
        question: e.question, edgeGrade: e.edgeGrade, edgeDirection: e.edgeDirection, divergence: e.divergence,
        sourceCount: e.sourceCount, recordedAt: e.recordedAt, marketPrice: e.marketPrice, ourProb: e.ourProb,
      }));
      const resolvedEdges = resolved.sort((a, b) => new Date(b.resolvedAt) - new Date(a.resolvedAt)).map(e => ({
        question: e.question, edgeGrade: e.edgeGrade, edgeDirection: e.edgeDirection,
        outcome: e.outcome === 1 ? 'YES' : 'NO', pnlPp: e.pnlPp, weWereBetter: e.weWereBetter,
        ourProb: e.ourProb, marketPrice: e.marketPrice, resolvedAt: e.resolvedAt, recordedAt: e.recordedAt,
      }));
      const timeline = {};
      for (const e of allEdges) { const d = e.recordedAt?.slice(0, 10); if (d) { timeline[d] = timeline[d] || { recorded: 0, resolved: 0 }; timeline[d].recorded++; if (e.resolved) timeline[d].resolved++; } }
      res.json({
  ...
  router.post('/intel/check-resolutions', async (req, res) => {
    ...
    try {
      const result = await edgeResolver.checkResolutions(async (conditionId) => {
        ...
        // Fallback: fetch directly from Polymarket API (catches resolved markets not in cache)
        try { if (client) { const apiMarket = await client.getMarketById(conditionId); if (apiMarket) return apiMarket; } } catch { /* non-critical */ }
        return null;
      });
      res.json(result);
    } catch (err) { res.status(500).json({ error: err.message }); }
  ...
  // ─── PnL & Trade History ───────────────────────────────────────────
  router.get('/pnl/chart', (req, res) => {
    try {
      if (!edgeResolver) return res.json({ trades: [], daily: [], summary: {} });
      res.json(edgeResolver.getPnLChart());
    } catch (err) { res.status(500).json({ error: err.message }); }
  });

  router.get('/trades/history', (req, res) => {
    try {
      if (!edgeResolver) return res.json({ trades: [], total: 0 });
      const status = req.query.status || 'all';
      res.json(edgeResolver.getTradeHistory(status));
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/intel/webhooks', (req, res) => {
    res.json({
      enabled: webhooks.config.enabled, urlCount: webhooks.config.urls.length,
      minQuality: webhooks.config.minQuality, minEdge: webhooks.config.minEdge,
  ...
  router.post('/intel/webhooks', (req, res) => {
    ...
    if (url && !webhooks.config.urls.includes(url)) webhooks.config.urls.push(url);
